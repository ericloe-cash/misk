...
internal class RealOpaPolicyEngineTest {
  ...
  val module: Module =
    object : KAbstractModule() {
      ...
      @Provides @Singleton fun opaConfig(): OpaConfig = OpaConfig("fake", null, true, true)

      @Provides @Singleton fun opaApi(): OpaApi = mock()

      @Provides @Singleton @Named("opa-moshi") fun provideMoshi(): Moshi = defaultKotlinMoshi
    }
  ...
  fun pojoInputQuery() {
    ...
    whenever(opaApi.queryDocument(anyString(), requestCaptor.capture(), anyBoolean(), anyBoolean()))
      .thenReturn(
        Calls.response(
          "{\"decision_id\": \"decisionIdString\", \"result\": {\"test\": \"a\"}}"
            .toResponseBody(APPLICATION_JSON.asMediaType())
  ...
  fun responseIsNotOk() {
    whenever(opaApi.queryDocument(anyString(), anyString(), anyBoolean(), anyBoolean()))
      .thenReturn(Calls.response(Response.error(403, "Access Denied".toResponseBody(APPLICATION_JSON.asMediaType()))))

    val exception =
      assertThrows<PolicyEngineException> {
        val evaluate: BasicResponse = opaPolicyEngine.evaluate("test", BasicRequest(1))
      }
  ...
  fun unknownPolicyDocument() {
    whenever(opaApi.queryDocument(anyString(), anyString(), anyBoolean(), anyBoolean()))
      .thenReturn(
        Calls.response("{\"decision_id\": \"decisionIdString\"}".toResponseBody(APPLICATION_JSON.asMediaType()))
      )
