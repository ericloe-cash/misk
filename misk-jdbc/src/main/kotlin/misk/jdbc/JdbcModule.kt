 ...
 *
 * This also registers services to connect to the database ([DataSourceService]) and to verify that the schema is
 * up-to-date ([SchemaMigratorService]).
 */
class JdbcModule
@JvmOverloads
constructor(
  ...
  override fun configure() {
    ...
    bind(keyOf<StartDatabaseService>(qualifier))
      .toProvider(
        object : Provider<StartDatabaseService> {
          @Inject lateinit var deployment: Deployment

          override fun get(): StartDatabaseService {
            return StartDatabaseService(deployment = deployment, config = config, qualifier = qualifier).init()
          }
        }
      )
      .asSingleton()
    ...
    if (installSchemaMigrator && config.migrations_format != MigrationsFormat.EXTERNALLY_MANAGED) {
      ...
      bind(schemaMigratorKey)
        .toProvider(
          object : Provider<SchemaMigrator> {
            @Inject lateinit var resourceLoader: ResourceLoader

            override fun get(): SchemaMigrator =
              when (config.migrations_format) {
      ...
      bind(schemaMigratorServiceKey)
        .toProvider(
          object : Provider<SchemaMigratorService> {
            @Inject lateinit var deployment: Deployment

            override fun get(): SchemaMigratorService =
  ...
  private fun bindDataSource(qualifier: KClass<out Annotation>, config: DataSourceConfig, isWriter: Boolean) {
    ...
    if (installHealthCheck) {
      bind(keyOf<PingDatabaseService>(qualifier))
        .toProvider { PingDatabaseService(config, deploymentProvider.get()) }
        .asSingleton()
    ...
    if (config.type == DataSourceType.VITESS_MYSQL) {
      ...
      bind(spanInjectorDecoratorKey)
        .toProvider(
          object : Provider<SpanInjector> {
            @com.google.inject.Inject(optional = true) var tracer: Tracer? = null

            override fun get(): SpanInjector = SpanInjector(tracer, config)
          }
        )
        .asSingleton()
    }
    ...
    if (config.type == DataSourceType.MYSQL && config.mysql_use_aws_secret_for_credentials) {
      ...
      bind(tracingDecoratorKey)
        .toProvider(
          object : Provider<TracingDataSourceDecorator> {
            @com.google.inject.Inject(optional = true) var tracer: Tracer? = null

            override fun get(): TracingDataSourceDecorator = TracingDataSourceDecorator(tracer, config)
          }
        )
        .asSingleton()
    }
