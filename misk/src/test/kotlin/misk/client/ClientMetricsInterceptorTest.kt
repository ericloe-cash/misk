...
internal class ClientMetricsInterceptorTest {
  ...
  fun responseCodes() {
    ...
    /**
     * In [Http1ExchangeCodec] Trailers() is null unless the response body uses chunked transfer-encoding and includes
     * trailers
     */
    mockWebServer.enqueue(
      MockResponse()
  ...
  fun readTrailersOnHttp2() {
    ...
    mockWebServer.enqueue(MockResponse().setResponseCode(200).setBody("{}").setTrailers(headersOf("grpc-status", "13")))

    mockWebServer.enqueue(MockResponse().setResponseCode(200).setBody("{}").setTrailers(headersOf("grpc-status", "0")))

    mockWebServer.enqueue(MockResponse().setResponseCode(200).setBody("{}").setTrailers(headersOf("grpc-status", "3")))
  ...
  fun urlAsAction() {
    ...
    val targetUrl =
      mockWebServer.url(
        "/path/to/test///?some&get=params"
      ) // leading/trailing slashes and GET params will be dropped in the interceptor
        ...
        endpoints =
          mapOf(
            ...
            "pingerDelay" to
              HttpClientEndpointConfig(
                url = url.toString(),
                clientConfig = HttpClientConfig(readTimeout = Duration.ofSeconds(5)),
              ),
            "pingerHttp2" to
              HttpClientEndpointConfig(
                url = url.toString(),
