...
internal class RequestDeadlineMetrics @Inject internal constructor(metrics: PalMetrics) {
  ...
  fun recordDeadlinePropagated(action: Action, timeout: Duration, timeoutSource: String) {
    ...
    deadlineDistributionHistogram.labels(action.name, timeoutSource, protocol).observe(timeout.toMillis().toDouble())
  }
  ...
  fun recordDeadlineExceeded(action: Action, direction: String, enforced: Boolean, timePassedSinceDeadlineMs: Long) {
    ...
    val protocol = determineProtocol(action.dispatchMechanism)
    deadlineExceededTimeHistogram
      .labels(action.name, direction, enforced.toString(), protocol)
      .observe(timePassedSinceDeadlineMs.toDouble())
  }

  fun recordOutboundDeadlineExceeded(
    clientAction: ClientAction,
    enforced: Boolean,
    isGrpc: Boolean,
    timePassedSinceDeadline: Duration,
  ) {
    val protocol = if (isGrpc) "grpc" else "http"
    deadlineExceededTimeHistogram
      .labels(clientAction.name, "outbound", enforced.toString(), protocol)
      .observe(timePassedSinceDeadline.toMillis().toDouble())
  }

  fun recordOutboundDeadlinePropagated(
    clientAction: ClientAction,
    deadline: Duration,
    isGrpc: Boolean,
    source: String,
    ...
    val protocol = if (isGrpc) "grpc" else "http"
    deadlineDistributionHistogram.labels(clientAction.name, source, protocol).observe(deadline.toMillis().toDouble())
  }

  fun recordNoDeadlineInScope(clientAction: ClientAction, isGrpc: Boolean) {
    val protocol = if (isGrpc) "grpc" else "http"
    noDeadlineInScopeCounter.labels(clientAction.name, protocol).inc()
  }
